...
def _make_handler(
    ...
    *,
    ...
    registered_tools: Any | None = None,
...
@pytest.mark.asyncio
async def test_registered_tools_route_returns_live_registry_snapshot(bus: MagicMock) -> None:
    channel = _ch(
        bus,
        port=29938,
        runtime_model_name=lambda: "qwen3-32b",
    ...
    await asyncio.sleep(0.3)
    try:
        deny = await _http_get("http://127.0.0.1:29938/api/webui/tools")
        assert deny.status_code == 401

        token = channel.gateway.tokens.issue_api_token(300)
        response = await _http_get(
            "http://127.0.0.1:29938/api/webui/tools",
