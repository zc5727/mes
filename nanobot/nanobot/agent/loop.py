...
from nanobot.agent.knowledge_grounding import (
    append_exact_web_sources,
    build_online_knowledge_block,
)
...
class AgentLoop:
    ...
    async def _state_build(self, ctx: TurnContext) -> str:
        ...
        grounding_block = await build_online_knowledge_block(ctx.msg.content)
        if grounding_block is not None:
            ctx.runtime_context_blocks.append(grounding_block)
    ...
    async def _state_run(self, ctx: TurnContext) -> str:
        ...
        generated_content = ctx.final_content
        ctx.final_content = append_exact_web_sources(
            ctx.final_content,
            ctx.runtime_context_blocks,
        )
        if ctx.final_content != generated_content:
            for message in reversed(ctx.all_messages):
                if (
                    message.get("role") == "assistant"
                    and message.get("content") == generated_content
                ):
                    message["content"] = ctx.final_content
                    break
        return "ok"
