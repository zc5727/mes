...
class GatewayHTTPHandler:
    ...
    def __init__(
        ...
        *,
        ...
        registered_tools: Callable[[], list[dict[str, str]]] | None = None,
        runtime_surface: str,
    ...
    def _handle_webui_tools(self, request: WsRequest) -> Response:
        if not self.check_api_token(request):
            return _http_error(401, "Unauthorized")
        tools = self.registered_tools() if self.registered_tools is not None else []
        return _http_json_response(
            {
                "count": len(tools),
                "model": self.runtime_model_name() if self.runtime_model_name else None,
                "tools": tools,
            }
        )
