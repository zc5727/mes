...
**负责人：** 赵丞  
**适用版本：** 核心 MES 试点版 v1.2 / 提交 `e5a340f2`
...
以下清单按当前提交 `e5a340f2` 的代码和脚本核对；“基础接口/演示”不等于生产完成。每一项只有在右栏证据归档后才能关闭。
...
截至提交 `e5a340f2`，质量/维修/追溯已有部分自动化契约证据，但以下事项仍不能标记为完成：ERPNext 真实旁路与四线对账、PostgreSQL 生产启用后的事务/重启恢复、ThingsBoard/Gateway 运行接入、质量/维修/库存/追溯完整持久化、正式身份与不可篡改审计、Tauri `.app/.dmg` 实机升级回滚。具体退出标准以 `docs/MES里程碑计划.md` 和 `docs/MES成熟功能域路线与端到端验收.md` 为准。
