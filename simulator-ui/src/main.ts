...
type FaultType = 'OVERHEAT' | 'JAM' | 'COMMUNICATION_LOSS' | 'QUALITY_DRIFT' | 'EMERGENCY_STOP' | 'MATERIAL_SHORTAGE' | 'QUALITY_ANOMALY';

const faultOptions: ReadonlyArray<{ value: FaultType; label: string }> = [
  { value: 'OVERHEAT', label: '设备过热' },
  { value: 'JAM', label: '设备卡料' },
  { value: 'COMMUNICATION_LOSS', label: '通信中断' },
  { value: 'QUALITY_DRIFT', label: '质量漂移' },
  { value: 'EMERGENCY_STOP', label: '紧急停机' },
...
function detailMarkup(): string {
  ...
  return `<div class="panel-title"><div><b>${device.name}</b><span class="tag">${device.status ?? 'unknown'}</span></div><span>${device.id}</span></div>
    ...
    <h3>设备故障</h3><div class="faults">${faultOptions.map(({ value, label }) => `<button data-fault="${value}">${label}</button>`).join('')}</div>`;
}
