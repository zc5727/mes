...
export class MaintenanceController {
  ...
  @Post('preventive-plans') async createPlan(@TenantId() tenantId: string, @Body() dto: CreatePreventivePlanDto, @Headers('x-user-id') actorId?: string) { return { data: await this.service.createPreventivePlanReliable(tenantId, dto, actorId), tenantId }; }
  @Get('spare-parts') listParts(@TenantId() tenantId: string) { return { data: this.service.listSpareParts(tenantId), tenantId }; }
  @Post('spare-parts') async createPart(@TenantId() tenantId: string, @Body() dto: CreateSparePartDto, @Headers('x-user-id') actorId?: string) { return { data: await this.service.createSparePartReliable(tenantId, dto, actorId), tenantId }; }
  @Post('spare-parts/consume') async consumePart(@TenantId() tenantId: string, @Body() dto: ConsumeSparePartDto, @Headers('x-user-id') actorId?: string) { return { data: await this.service.consumeSparePartReliable(tenantId, dto, actorId), tenantId }; }
  @Post('spare-parts/return') async returnPart(@TenantId() tenantId: string, @Body() dto: ConsumeSparePartDto, @Headers('x-user-id') actorId?: string) { return { data: await this.service.returnSparePartReliable(tenantId, dto, actorId), tenantId }; }
